...
```{r}
poly.area <- with(P102, vowelMeansPolygonArea(f1, f2, ipa, poly.order = c("i", 
    "ɛ", "ɑ", "o", "u", "æ", "ɪ", "ʌ"), group = ppt))
hull.area <- with(P102, convexHullArea(f1, f2, group = ppt))
...
```{r}
par(mfrow = c(2, 2))
with(P102, plotVowels(f1, f2, var.sty.by = ipa, var.col.by = ipa, pretty = TRUE))
```

Add the IPA symbols to the visualization.

```{r}
with(P102, plotVowels(f1, f2, ipa, plot.tokens = TRUE, pch.tokens = ipa, 
...
```{r}
with(P102, plotVowels(f1, f2, ipa, plot.tokens = TRUE, pch.tokens = ipa, cex.tokens = 1.2, alpha.tokens = 0.2, plot.means = TRUE, pch.means = ipa, cex.means = 2, var.col.by = ipa, ellipse.line = TRUE, pretty = TRUE))
```

Here is another version:

```{r}
with(P102, plotVowels(f1, f2, ipa, group = ipa, plot.tokens = FALSE, plot.means = TRUE, 
